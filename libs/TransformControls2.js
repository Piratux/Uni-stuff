/**
 * @author (original) arodic / https://github.com/arodic
 * 
 * Modifications made by Piratux.
 * This is a modified version of TransformControls.js from https://github.com/mrdoob/three.js/blob/r114/examples/js/controls/TransformControls.js
 */

THREE.TransformControls = function ( camera, domElement, selectable_objects ) {
	...
	if ( selectable_objects === undefined ) {

		console.warn( 'THREE.TransformControls: The third parameter "selectable_objects" is undefined.' );
		domElement = document;
	...
	this.objects = selectable_objects;
	...
	defineProperty( "defaultControlsEnabled", true );
	defineProperty( "onDrag", undefined ); // function
	defineProperty( "onDragStart", undefined ); // function
	defineProperty( "onAttach", undefined ); // function
	defineProperty( "forceShowAxis", undefined ); // array of strings that match handle names
	defineProperty( "forceHideAxis", undefined ); // array of strings that match handle names
	defineProperty( "useDefaultTranslation", true ); // bool
	defineProperty( "showInfiniteAxisLineDisplay", true ); // bool
	...
	var rotationStart = new THREE.Vector3();
	var quaternionStart = new THREE.Quaternion();
	var scaleStart = new THREE.Vector3();
	var worldQuaternionInvStart = new THREE.Quaternion();
	var _planeStart = new THREE.TransformControlsPlane();
	var startDirectionX = new THREE.Vector3(); // starting direction of X gizmo axis 
	var startDirectionY = new THREE.Vector3(); // starting direction of X gizmo axis 
	var startDirectionZ = new THREE.Vector3(); // starting direction of X gizmo axis 
	...
	this.pointerDown = function ( pointer ) {
		...
		if ( this.object !== undefined && ( pointer.button === 0 || pointer.button === undefined ) && this.axis !== null ) {
			...
			if ( planeIntersect ) {
				...
				rotationStart.copy( this.object.rotation );
				quaternionStart.copy( this.object.quaternion );
				scaleStart.copy( this.object.scale );
				
				_planeStart.copy(_plane);
				
				var x_vector = new THREE.Vector3(1, 0, 0);
				var y_vector = new THREE.Vector3(0, 1, 0);
				var z_vector = new THREE.Vector3(0, 0, 1);
				
				// NOTE: This needs to change if gizmo["translate"] object order changes or some elements get added/removed
				var handle_x = _gizmo.gizmo[ "translate" ].children[0];
				var handle_y = _gizmo.gizmo[ "translate" ].children[3];
				var handle_z = _gizmo.gizmo[ "translate" ].children[6];
				
				startDirectionX = x_vector.applyQuaternion(handle_x.quaternion).normalize();
				startDirectionY = y_vector.applyQuaternion(handle_y.quaternion).normalize();
				startDirectionZ = z_vector.applyQuaternion(handle_z.quaternion).normalize();

				this.object.matrixWorld.decompose( worldPositionStart, worldQuaternionStart, worldScaleStart );
				
				worldQuaternionInvStart.copy( worldQuaternionStart ).inverse(); // extra
			...
			if(this.onDragStart !== undefined){
				this.onDragStart(this.object, this.axis);
			}

		}
		else{
			this.trySelectObject(pointer);
		}

	};
	
	this.trySelectObject = function ( pointer ) {
		
		if (this.objects === undefined) return;
		
		ray.setFromCamera( pointer, this.camera );
		var intersect = ray.intersectObjects( this.objects )[ 0 ] || false;
		if ( intersect ) {
			
			this.attach(intersect.object);
	...
	this.pointerMove = function ( pointer ) {
		...
		// Library doesn't expect that we can rotate object when translating it, so have to cheat a bit
		var planeIntersect = ray.intersectObjects( [ _planeStart ], true )[ 0 ] || false;
		...
		if ( mode === 'translate' ) {
			...
			if ( space === 'local' && axis !== 'XYZ' ) {
				...
				offset.applyQuaternion( worldQuaternionInvStart );
			...
			if (this.useDefaultTranslation === true){
			...
			if (this.onDrag !== undefined){
				...
				mat3.set(
					startDirectionX.x, startDirectionY.x, startDirectionZ.x,
					startDirectionX.y, startDirectionY.y, startDirectionZ.y,
					startDirectionX.z, startDirectionY.z, startDirectionZ.z
				);
				
				mat3.getInverse(mat3);
				
				var start = new THREE.Vector3();
				var end = new THREE.Vector3();
				var diff = new THREE.Vector3();
				
				start.copy(positionStart).applyMatrix3(mat3);
				end.copy(positionStart).add(offset).applyMatrix3(mat3);
				diff.copy(end).sub(start);
				
				this.onDrag(object, this.axis, diff, positionStart, rotationStart, scaleStart);
			}
	...
	function onKeyDown( event ) {
		...
		if ( ! scope.defaultControlsEnabled ) return;
		...
		case 89: // Y
			...
			break;
		case 90: // Z
			scope.showZ = !scope.showZ;
			break;
		case 32: // Spacebar
			scope.enabled = !scope.enabled;
			break;
		case 17: // Ctrl
			scope.translationSnap = 10;
			scope.rotationSnap = 0.2617; // 15 degrees
			scope.scaleSnap = 0.2;
			break;
		case 187:
		case 107: // +, =, num+
	...
	function onKeyUp( event ) {
		...
		switch (event.keyCode) {
		case 17: // Ctrl
			scope.translationSnap = null;
			scope.rotationSnap = null;
			scope.scaleSnap = null;
			break;
		}

	}
	
	this.printDefaultControls = function() {

		var info = 
			'Click on object to show its gizmo.\n' +
			'Controls:\n' +
			'        W: Switch to translate mode\n' +
			'        E: Switch to rotate mode\n' +
			'        R: Switch to scale mode\n' +
			'        Q: Toggle global/local rotation mode\n' +
			'        X: Toggle X axis\n' +
			'        Y: Toggle Y axis\n' +
			'        Z: Toggle Z axis\n' +
			'    Space: Enable/disable controls\n' +
			'     Ctrl: Hold to enable snapping\n' +
			'+ OR num+: Increase gizmo size\n' +
			'- OR num-: Decrease gizmo size\n' +
			'      LMB: Select object to show object\'s gizmo'
		;
...
THREE.TransformControlsGizmo = function () {
	...
	this.updateMatrixWorld = function () {
		...
		// var quaternion = this.worldQuaternion;
		...
		for ( var i = 0; i < handles.length; i ++ ) {
			...
			if ( handle.tag === 'helper' ) {
				...
				if ( handle.name === 'AXIS' ) {
					...
					if ( this.axis && this.showInfiniteAxisLineDisplay ) {
			...
			if(this.forceShowAxis !== undefined){
				for(var j = 0; j < this.forceShowAxis.length; j++){
					if(this.forceShowAxis[j] == handle.name){
						handle.visible = true;
					}
				}
			}
			
			if(this.forceHideAxis !== undefined){
				for(var j = 0; j < this.forceHideAxis.length; j++){
					if(this.forceHideAxis[j] == handle.name){
						handle.visible = false;
					}
				}
			}
